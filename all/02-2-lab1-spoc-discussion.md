...
> * 初始化8259中断控制器，完成对8259的一系列配置，相应外设就能产生中断，被CPU接受和处理
> * 初始化中断描述符表IDT，为中断号和中断服务例程（包括段描述符和偏移地址）建立索引，可以根据中断号查找相应服务例程的具体函数位置
> * 初始化时钟中断，进行相关外设配置
> * 使能中断，调用一个特殊指令开启中断功能
...
> 先实现一个能从实模式转化为保护模式的bootloader，支持串口、时钟、CGA等访问，实现对字符串的输入；再实现一个能加载ELF格式的bootloader，能解析ELF头格式，加载相应扇区内容到内存中，并将控制权交给ucore，需要保证其大小小于512B
...
> * 使用linux的strace命令
> * 可以使用类似lab8中的syscall.c的程序，添加计数器统计ucore使用的所有系统调用

如何裁减lab1, 实现一个可显示字符串"THU LAB1"且依然能够正确加载ucore OS的bootloader？如果不能完成实现，请说明理由。
- [x]  

> 不知道是否可以实现，因为现实字符串需要一定代码量，而在原来基础上添加一定代码量可能会超过512B
